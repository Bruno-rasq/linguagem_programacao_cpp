...
namespace movimenthandler
{
    ...
    void Projectil::updateCoord()
    {
        ...
        if (this->dir == 1)
            this->coord.delta(-1, 0);
        if (this->dir == 2)
            this->coord.delta(1, 0);
        if (this->dir == 3)
            this->coord.delta(0, -1);
        if (this->dir == 4)
            this->coord.delta(0, 1);
    };
    ...
    {
        return (0 <= coord.x && coord.x < FRAME_BOARD_MAX_HEIGTH && 0 <= coord.y && coord.y < FRAME_BOARD_MAX_WIDTH);
    };

    void wrap_around(int8_t &x, int8_t &y)
    {
        if (x > FRAME_BOARD_MAX_HEIGTH - 1)
            x = 0;
        if (x < 0)
            x = FRAME_BOARD_MAX_HEIGTH - 1;
        if (y > FRAME_BOARD_MAX_WIDTH - 1)
            y = 0;
        if (y < 0)
            y = FRAME_BOARD_MAX_WIDTH - 1;
    };
}